...
def test_spell_checker_no_conn_empty_corrections():
    """No pg_conn → SpellChecker.corrections is empty dict."""
    sc = SpellChecker({"threshold": 0.85})
    assert sc.corrections == {}


def test_spell_checker_db_error_falls_back_to_empty():
    """DB error during load → corrections empty, no exception raised."""
    with patch("cleaning.spell_corrections_data.get_corrections_dict", side_effect=Exception("DB down")):
        mock_conn = MagicMock()
        sc = SpellChecker({"pg_conn": mock_conn, "threshold": 0.85})
    assert sc.corrections == {}


def test_spell_checker_with_db_corrections():
    """With DB corrections injected, SpellChecker corrects misspellings."""
    ...
    assert result["address"] == "123 Main St"  # unchanged
    assert "_decisions" in result
    assert len(result["_decisions"]) == 2
